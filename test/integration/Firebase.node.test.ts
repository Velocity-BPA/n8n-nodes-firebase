...
import { Firebase } from '../../nodes/Firebase/Firebase.node';

describe('Firebase Node Integration', () => {
	let firebaseNode: Firebase;

	beforeEach(() => {
		firebaseNode = new Firebase();
	});
	...
	describe('Node Configuration', () => {
		...
		it('should have all 7 resources', () => {
			const resourceProperty = firebaseNode.description.properties.find(
				(p) => p.name === 'resource',
			);
			expect(resourceProperty).toBeDefined();
			expect(resourceProperty?.options).toHaveLength(7);

			const resourceNames = (resourceProperty?.options as any[]).map((o) => o.value);
	...
	describe('Realtime Database Resource', () => {
		it('should have all required operations', () => {
			const operationProperty = firebaseNode.description.properties.find(
				(p) => p.name === 'operation' && p.displayOptions?.show?.resource?.includes('realtimeDatabase'),
			);
			expect(operationProperty).toBeDefined();

			const operations = (operationProperty?.options as any[]).map((o) => o.value);
	...
	describe('Firestore Resource', () => {
		it('should have all required operations', () => {
			const operationProperty = firebaseNode.description.properties.find(
				(p) => p.name === 'operation' && p.displayOptions?.show?.resource?.includes('firestore'),
			);
			expect(operationProperty).toBeDefined();
	...
	describe('Authentication Resource', () => {
		it('should have all required operations', () => {
			const operationProperty = firebaseNode.description.properties.find(
				(p) => p.name === 'operation' && p.displayOptions?.show?.resource?.includes('auth'),
			);
			expect(operationProperty).toBeDefined();
	...
	describe('Cloud Messaging Resource', () => {
		it('should have all required operations', () => {
			const operationProperty = firebaseNode.description.properties.find(
				(p) => p.name === 'operation' && p.displayOptions?.show?.resource?.includes('messaging'),
			);
			expect(operationProperty).toBeDefined();
	...
	describe('Remote Config Resource', () => {
		it('should have all required operations', () => {
			const operationProperty = firebaseNode.description.properties.find(
				(p) => p.name === 'operation' && p.displayOptions?.show?.resource?.includes('remoteConfig'),
			);
			expect(operationProperty).toBeDefined();
	...
	describe('Dynamic Links Resource', () => {
		it('should have all required operations', () => {
			const operationProperty = firebaseNode.description.properties.find(
				(p) => p.name === 'operation' && p.displayOptions?.show?.resource?.includes('dynamicLinks'),
			);
			expect(operationProperty).toBeDefined();
	...
	describe('Cloud Storage Resource', () => {
		it('should have all required operations', () => {
			const operationProperty = firebaseNode.description.properties.find(
				(p) => p.name === 'operation' && p.displayOptions?.show?.resource?.includes('storage'),
			);
			expect(operationProperty).toBeDefined();
	...
	describe('Default Values', () => {
		it('should have sensible default values for pagination', () => {
			const pageSizeProperty = firebaseNode.description.properties.find(
				(p) => p.name === 'pageSize',
			);
		...
		it('should have sensible default values for limits', () => {
			const limitProperty = firebaseNode.description.properties.find(
				(p) => p.name === 'limit',
			);
